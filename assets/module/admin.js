...
layui.define(['layer', 'setter', 'layRouter'], function (exports) {
    ...
    admin.popupRight = function (param) {
        ...
        param.move = false;
        param.fixed = true;
        (param.area === undefined) && (param.area = '336px');
        (param.title === undefined) && (param.title = false);
        (param.closeBtn === undefined) && (param.closeBtn = false);
        (param.shadeClose === undefined) && (param.shadeClose = true);
        (param.skin === undefined) && (param.skin = 'layui-anim layui-anim-rl layui-layer-adminRight');
        return admin.open(param);
    };

    /* 封装layer.open */
    admin.open = function (param) {
        (param.area === undefined) && (param.area = (param.type === 2 ? ['360px', '300px'] : '360px'));
        (param.offset === undefined) && (param.offset = '70px');
        (param.shade === undefined) && (param.shade = .1);
        (param.fixed === undefined) && (param.fixed = false);
        (param.resize === undefined) && (param.resize = false);
        (param.skin === undefined) && (param.skin = 'layui-layer-admin');
        ...
        if (param.url) {
            ...
            param.success = function (layero, index) {
                admin.reloadLayer(layero, param.url, sCallBack);
            };
        }
    ...
    admin.reloadLayer = function (index, url, success) {
        ...
        (url === undefined) && (url = $layero.data('url'));
        $layero.data('url', url);
    ...
    admin.req = function (url, data, success, method) {
    ...
    admin.ajax = function (param) {
        ...
        param.success = function (result, status, xhr) {
            ...
            (before !== false && success) && success(result, status, xhr);
        };
        param.error = function (xhr) {
            param.success({ code: xhr.status, msg: xhr.statusText }, undefined, xhr);
        };
        // 解决缓存问题
        if (layui.cache.version && (!setter.apiNoCache || param.dataType.toLowerCase() !== 'json')) {
            ...
            if (layui.cache.version === true) {
                param.url += new Date().getTime();
            } else {
                param.url += layui.cache.version;
            }
        }
        $.ajax(param);
    };
    ...
    admin.showLoading = function (elem, type, opacity, size) {
        ...
        (opacity !== undefined) && ($loading.css('background-color', 'rgba(255,255,255,' + opacity + ')'));
        $loading.show();
    };
    ...
    /* 缓存临时数据 */
    admin.putTempData = function (key, value) {
    ...
    /* 获取缓存临时数据 */
    admin.getTempData = function (key) {
        var tableName = setter.tableName + '_tempData';
    ...
    admin.changeTheme = function (theme, win, noCache, noChild) {
        ...
        if (!noChild) {
            ...
            for (var i = 0; i < ifs.length; i++) {
                admin.changeTheme(theme, ifs[i], true, true);
            }
        }
    };
    ...
    admin.resizeTable = function (time) {
        setTimeout(function () {
            var $tbView = setter.pageTabs ? $(tabDOM + '>.layui-tab-content>.layui-tab-item.layui-show') : $(bodyDOM);
    ...
    admin.events = {
        ...
        theme: function () {
            ...
            admin.popupRight({
                id: 'layer-theme',
                url: url ? url : 'components/tpl/theme.html'
            });
        },
        /* 打开便签 */
        note: function () {
        ...
        message: function () {
            ...
            admin.popupRight({
                id: 'layer-notice',
                url: url ? url : 'components/tpl/message.html'
            });
        },
        /* 打开修改密码弹窗 */
        psw: function () {
            ...
            admin.open({
                ...
                title: '修改密码',
                shade: 0,
                url: url ? url : 'components/tpl/password.html'
            });
        },
        /* 退出登录 */
        logout: function () {
        ...
        open: function () {
            ...
            admin.open(admin.parseLayerOption(option));
        },
        /* 打开右侧弹窗 */
        popupRight: function () {
            var option = admin.util.deepClone($(this).data());
            admin.popupRight(admin.parseLayerOption(option));
        },
        /* 全屏 */
        fullScreen: function () {
    ...
    admin.lockScreen = function (url) {
        url || (url = 'components/tpl/lock-screen.html');
        var $lock = $('#ew-lock-screen-group');
        if ($lock.length > 0) {
            $lock.fadeIn('fast');
            admin.isLockScreen = true;
            admin.putTempData('isLockScreen', admin.isLockScreen);
        } else {
            var loadIndex = layer.load(2);
            admin.ajax({
                url: url,
                ...
                success: function (res) {
                    ...
                    admin.isLockScreen = true;
                    admin.putTempData('isLockScreen', admin.isLockScreen);
                    admin.putTempData('lockScreenUrl', url);
                }
            });
        }
    };
    ...
    admin.unlockScreen = function (isRemove) {
        ...
        admin.putTempData('isLockScreen', undefined);
    };
    ...
    admin.putSetting = function (key, value) {
        ...
        layui.data(setter.tableName, {
            key: key,
            value: value,
            remove: value === undefined || value === null
        });
    };
    ...
    admin.recoverState = function () {
        // 恢复锁屏状态
        if (admin.getTempData('isLockScreen')) {
            admin.lockScreen(admin.getTempData('lockScreenUrl'));
        }
        // 恢复配置的主题
        if (setter.defaultTheme) {
            admin.changeTheme(setter.defaultTheme, window, true, true);
        }
        // 恢复页脚状态、导航箭头
        setter.closeFooter && $('body').addClass('close-footer');
        if (setter.navArrow !== undefined) {
            var $nav = $(sideDOM + '>.layui-nav-tree');
            $nav.removeClass('arrow2 arrow3');
            setter.navArrow && $nav.addClass(setter.navArrow);
        }
    };
    ...
    $(document).on('click', '.form-search-expand', function () {
        ...
        if ($form.hasClass('form-search-is-expand')) {
            $form.removeClass('form-search-is-expand');
            $(this).html('展开 <i class="layui-icon layui-icon-down"></i>');
        } else {
            $form.addClass('form-search-is-expand');
    ...
    if (cache) {
        var keys = ['pageTabs', 'cacheTab', 'defaultTheme', 'navArrow', 'closeFooter'];
        for (var i = 0; i < keys.length; i++) {
            (cache[keys[i]] !== undefined) && (setter[keys[i]] = cache[keys[i]]);
        }
    }
    admin.recoverState();
